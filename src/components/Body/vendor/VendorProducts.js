    ...
    <div className="max-w-7xl mx-auto bg-white p-8 my-8 rounded-md shadow-lg">
      ...
      <div className="grid grid-cols-2 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-3 gap-8">
        {products.map((product) => (
          ...
          <div
            key={product._id}
            className="bg-gray-100 p-4 rounded-md border border-gray-100 hover:border-blue-500 hover:rounded hover:border-1 hover:duration-300 hover:ease-in-out"
          >
            <Link to={"/vendor/product/" + product?._id}>
              <img
            ...
            <p className="text-gray-700 mb-2 font-medium">
              Available Quantity:{" "}
              <span className="text-violet-500">{product?.available_Quantity}</span>
            </p>
          </div>
        ))}
      </div>
