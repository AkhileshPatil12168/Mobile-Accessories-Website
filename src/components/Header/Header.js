...
const UserNavBar = ({ cUserId, cToken }) => {
  return (
    <ul className="flex flex-col font-medium p-4 md:p-0 mt-4 border border-gray-100 rounded-lg bg-gray-50 md:flex-row md:space-x-8 md:mt-0 md:border-0 md:bg-white dark:bg-gray-800 md:dark:bg-gray-900 dark:border-gray-700">
      ...
      <Link
        className="block py-2 pl-3 pr-4 text-gray-900 rounded hover:bg-gray-100 md:hover:bg-transparent md:hover:text-blue-700 md:p-0 md:dark:hover:text-blue-500 dark:text-white dark:hover:bg-gray-700 dark:hover:text-white md:dark:hover:bg-transparent dark:border-gray-700"
        to={cToken && cUserId ? "/wishlist" : "/login"}
      >
        <li className="px-2">Wishlist💖</li>
      </Link>
...
const AdminNavBar = ({ cToken, cAdminId }) => {
  return (
    <ul className="flex flex-col font-medium p-4 md:p-0 mt-4 border border-gray-100 rounded-lg bg-gray-50 md:flex-row md:space-x-8 md:mt-0 md:border-0 md:bg-white dark:bg-gray-800 md:dark:bg-gray-900 dark:border-gray-700">
      <Link
...
const VendorNavBar = (cVendorId, cToken) => {
  return( <></>);
};
const Header = () => {
    ...
    <>
      <nav className="bg-white dark:bg-gray-900 w-full  border-b border-gray-200 dark:border-gray-600">
        <div className="max-w-screen-xl flex flex-wrap items-center justify-between mx-auto p-4">
          <Title />
          {cAdminId ? (
            <AdminNavBar cAdminId={cAdminId} cToken={cToken} />
          ) : cVendorId ? (
            <VendorNavBar />
          ) : (
            <UserNavBar cUserId={cUserId} cToken={cToken} />
          )}

          {cToken && (cUserId || cAdminId || cVendorId) ? (
            <button
